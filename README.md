...
## Текущий этап — Phase 24.3

Phase 24.3 остаётся внутри направления файловых операций Phase 24: после `COPY` и `REN` shell получает `MOVE` для переноса файлов и каталогов между каталогами. Это небольшая подфаза без изменения модели процессов или syscall ABI.

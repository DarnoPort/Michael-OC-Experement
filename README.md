...
Старый `SYS_WRITE` сохраняется как совместимый legacy-интерфейс. Новые программы должны использовать `SYS_FD_WRITE`, чтобы вывод шёл через стандартный файловый дескриптор.
...
## Phase 25.4: Executable File State

Phase 25.4 отделяет обычные файлы от исполняемых объектов.

DiskFS v1 уже имел два неиспользуемых поля в inode. Теперь первое из них используется как persistent flags field. Формат inode не увеличивается и номер версии DiskFS не меняется.

Добавлен флаг:

`DISKFS_FLAG_EXECUTABLE`

VFS переносит этот признак из inode в память и предоставляет:

`vfs_node_is_executable()`
`vfs_set_executable()`
...
Команда `run` больше не пытается запускать любой файл подряд. Она принимает только файл, помеченный executable.

Это важно для дальнейшей архитектуры: расширение `.elf` само по себе больше не определяет тип объекта. Программа становится отдельной категорией файловой системы.

`ls` и `DIR /W` показывают такие объекты как `[EXEC]`.

Встроенные `install-demo`, `install-exec-test` и `install-args-test` теперь также выставляют executable flag после записи программы.

Обычный файл, импортированный через `disk-import`, можно оставить обычным, даже если его содержимое случайно является ELF. Такой файл `run` не запустит.

...
## Текущий этап — Phase 24.5

Phase 24.5 продолжает две небольшие линии текущего shell: терминал теперь позволяет редактировать команду за пределами одной строки с автоматическим переносом, а `MOVE` сохраняет корректный `cwd`, если перемещается каталог, внутри которого находится текущая директория. Изменения не требуют нового syscall ABI или изменения формата DiskFS.

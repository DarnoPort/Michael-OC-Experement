...
## Текущий этап — Phase 24.6

Phase 24.6 завершает очередной локальный шаг интерфейса терминала: экран теперь хранит историю прокрутки, поэтому старые строки не исчезают безвозвратно после заполнения 80x25. `PageUp` / `PageDown` позволяют перемещаться по истории, а любое обычное действие ввода автоматически возвращает просмотр в нижнюю часть терминала. Изменения не требуют нового syscall ABI или изменения формата DiskFS.

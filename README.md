...
## Текущий этап — Phase 24.2

Phase 24.2 остаётся внутри направления файловых операций Phase 24: после `COPY` shell получает `REN`/`RENAME` для переименования файлов. Это отдельная небольшая подфаза без изменения модели процессов или syscall ABI.
...
- каталоги пока не переименовываются через shell-команду;
...
`REN` меняет только имя inode. Содержимое файла, размер и inode остаются прежними.

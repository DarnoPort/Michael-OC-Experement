...
static unsigned int section_page_flags(unsigned int address) {
    ...
    unsigned int text_start = (unsigned int)(unsigned long)&_text_start;
    unsigned int text_end = (unsigned int)(unsigned long)&_text_end;
    unsigned int rodata_start = (unsigned int)(unsigned long)&_rodata_start;
    unsigned int rodata_end = (unsigned int)(unsigned long)&_rodata_end;
...
static unsigned int vm_find_free_run(unsigned int count) {
    if (count == 0 || count > KERNEL_VM_PAGES) {
        return VM_ALLOC_FAIL;
    }
...
void paging_print_info(void) {
    ...
    if (!paging_is_enabled()) {
        print_string("Paging is not enabled.\n", 0x0C);
        return;
    }
    ...
    print_string(" / ", 0x07);
    paging_print_uint(KERNEL_VM_PAGES, 0x0F);
    print_char('\n', 0x07);
}
...
void paging_test(void) {
    ...
    if (!paging_is_enabled()) {
        print_string("vmtest: paging is not enabled.\n", 0x0C);
        return;
    }

    print_string("Running virtual memory test...\n", 0x0E);

    address = vm_alloc_pages(2, PAGE_WRITABLE);
    if (address == VM_ALLOC_FAIL) {
        print_string("vmtest: allocation FAILED.\n", 0x0C);
        return;
    }

    physical0 = paging_get_physical(address);
    physical1 = paging_get_physical(address + PAGE_SIZE);

    print_string("Virtual:  ", 0x07);
    paging_print_hex32(address, 0x0F);
    print_string(" ", 0x07);
    paging_print_hex32(address + PAGE_SIZE, 0x0F);
    print_char('\n', 0x07);
    ...
    if (physical0 == VM_ALLOC_FAIL ||
        ...
        print_string("vmtest: translation FAILED.\n", 0x0C);
        vm_free_pages(address, 2);
        return;
    }

    values = (volatile unsigned int*)address;
    ...
    if (values[0] != 0x13579BDFU ||
        values[1023] != 0x2468ACE0U ||
        values[1024] != 0x55AA55AAU) {
        print_string("vmtest: read/write FAILED.\n", 0x0C);
        vm_free_pages(address, 2);
        return;
    }

    if (!vm_free_pages(address, 2)) {
        print_string("vmtest: free FAILED.\n", 0x0C);
        return;
    }

    if (paging_get_physical(address) != VM_ALLOC_FAIL) {
        print_string("vmtest: unmap FAILED.\n", 0x0C);
        return;
    }

    print_string("vmtest: PASS\n", 0x0A);
}
